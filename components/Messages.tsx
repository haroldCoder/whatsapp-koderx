    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1, marginBottom: 80 }}>
          <ScrollView contentContainerStyle={{ padding: 8, paddingLeft: 20, paddingTop: 16}}>
            {
              message && phone.current != "" && message.map((ms: message, index: number) => (
                <View key={index} style={{ backgroundColor: `${ms.number_em == phone.current ? "#28A" : "#333"}`, borderRadius: 10, padding: 12, alignSelf: "flex-start", marginBottom: 12 }}>
