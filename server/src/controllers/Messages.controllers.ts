...
class MessagesController extends ConectDB{
    ...
    ViewMessages = async(user_em: number, user_tr: number) =>{
        ...
        this.client.query(`SELECT content FROM messages JOIN users ON messages.Id_em = users.ID WHERE messages.Id_tr = ${this.user_em} AND messages.Id_em = ${this.user_em}`)
        .then((res)=>{
